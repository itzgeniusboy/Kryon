...
object AiCopilotService {
    ...
    suspend fun getClaudeCommandResponse(
        ...
        if (apiKey.trim().isEmpty()) {
            ...
            return@withContext parseHeuristically(context, userPrompt, currentPath)
        }
        ...
        try {
            ...
            parseHeuristically(context, userPrompt, currentPath)
        }
    }
    ...
    private fun parseHeuristically(context: Context, prompt: String, currentPath: String): CopilotResponse {
        ...
        if (normalized.contains("delete") || normalized.contains("remove") || normalized.contains("clean")) {
            ...
            val provider = SecurePreferences.getAiProvider(context)
            val providerName = if (provider == "GEMINI") "Gemini" else "Claude"
            explanation += "I heard '$prompt'. Configure $providerName API in settings to unlock full natural language commands!"
        }

        return CopilotResponse(explanation, actions)
    }

    // Generic abstraction to query whichever AI provider is selected
    suspend fun queryActiveText(
        context: Context,
        systemInstruction: String,
        userContent: String
    ): String {
        ...
        return if (provider == "GEMINI") {
            queryGeminiText(context, systemInstruction, userContent)
        } else {
            queryClaudeText(context, systemInstruction, userContent)
        }
    }

    // Generic abstraction to run commands using the active AI provider
    suspend fun getActiveCommandResponse(
        context: Context,
        userPrompt: String,
        currentPath: String,
        fileMetadata: String
        ...
        val provider = SecurePreferences.getAiProvider(context)
        return if (provider == "GEMINI") {
            getGeminiCommandResponse(context, userPrompt, currentPath, fileMetadata)
        } else {
            getClaudeCommandResponse(context, userPrompt, currentPath, fileMetadata)
        }
    }
    ...
    suspend fun queryGeminiText(
        ...
        val apiKey = SecurePreferences.getGeminiApiKey(context)
        if (apiKey.trim().isEmpty()) {
            return@withContext "Kryon AI offline. Please configure your Gemini API key in Settings."
        }
    ...
    suspend fun getGeminiCommandResponse(
        context: Context,
        userPrompt: String,
        currentPath: String,
        fileMetadata: String
        ...
        try {
            ...
            okHttpClient.newCall(request).execute().use { response ->
                val bodyStr = response.body?.string() ?: ""
                if (!response.isSuccessful) {
                    Log.e(TAG, "Gemini request failed with code ${response.code}: $bodyStr")
                    return@withContext parseHeuristically(context, userPrompt, currentPath)
                }
            ...
            Log.e(TAG, "Error executing Gemini request, failing back to heuristic parser", e)
            parseHeuristically(context, userPrompt, currentPath)
        }
    }
}
